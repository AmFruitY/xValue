...
seasons 2018/19 – 2022/23, then filters to players aged under 23.

Output: data/u23_players.csv
"""
...
def main() -> None:
    ...
    # Filter for players under 23
    u23 = merged[merged["age"] < 23].copy()
    u23 = u23.sort_values(["season", "league", "age"])

    out_path = OUTPUT_DIR / "u23_players.csv"
    u23.to_csv(out_path, index=False)

    print(f"\nDone. {len(u23)} player-season rows saved to '{out_path}'.")
    if "player" in u23.columns:
        print(u23[["season", "league", "player", "age"]].head(10).to_string(index=False))
